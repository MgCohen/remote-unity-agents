...
### Ids.Stamp → gives every block a distinct sequential handle
<!-- id: b28 -->
- **Why:** ids are handed out as a running `b<N>` counter, so every block gets a distinct handle by construction —
  the uniqueness the addressing depends on holds automatically, with no title-derived clash for the author to resolve.
...
### Ids.Stamp → the handle is a short opaque id carrying nothing of the title
<!-- id: b30 -->
- **Why:** the handle is a `b<N>` integer, not a title slug — it deliberately encodes nothing of the heading, so a
  reword or a reorder cannot perturb it, and a client's stored reference stays a short, stable, opaque key.
