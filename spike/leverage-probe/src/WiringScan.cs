...
// PROBE B at emit time (reused infra). The markers live in the handler body TEXT,
// so the emit tool parses that text and scans it for scope.Get<T>() /
// scope.Ask<T>() — additively, no body rewrite. T is resolved through the shared
// ResolutionModel. The output drives the emitted RegisterDiscovered(container).
//
// The motif assembles the full handler body (motif scaffold + author divergence)
// and hands it here, so the scan sees BOTH the motif's scope.Get<Repo<TAgg>> and
// any scope.Ask<T> the divergence uses — one uniform marker grammar.
