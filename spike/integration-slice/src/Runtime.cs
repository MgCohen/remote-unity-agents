namespace Slice.Runtime;

// The runtime DI seam the emitted feature runs against — the hand-authored half
// of probe B's additive wiring. `scope.Get<T>()` / `scope.Ask<T>()` are REAL
// container-backed calls (so the glue body compiles and runs as written) AND the
// exact call sites the wiring pass scans to emit RegisterDiscovered. No
// interceptors, no body rewrite.
//
// This is committed with the emitted feature so the proof builds + runs.
